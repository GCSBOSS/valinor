...
function buildValidationError(fn, input, r) {
...
class Valinor {
    ...
    test(input, fnContext) {
        ...
        let applyR = applyRules.bind(this)(this.rules, input, fnContext);
    ...
    assert(input, fnContext) {
        let testR = this.test(input, fnContext);
        ...
        if(testR instanceof Promise)
            return new Promise((resolve, reject) => {
                testR.then(r => {
                    if(r === true)
                        return resolve(true);
                    reject(buildValidationError(this.assert, input, r));
                });
            });

        if(testR === true)
            return true;
        throw buildValidationError(this.assert, input, testR);
    }
    ...
    fn(name, cb){
        this.rules.push({ custom: true, fn: cb, name, args: [] });
        return this;
    }
